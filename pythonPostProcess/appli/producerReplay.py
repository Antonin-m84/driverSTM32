...
def replay_data(queue: multiprocessing.Queue, file_path: str):
    def get_lines():
    ...
    def display_epoch(epoch_time):
        ...
        formatted_time = datetime.fromtimestamp(epoch_time_seconds).strftime('%Y/%m/%d      %Hh%M   %S.%f')
        text = small_font.render(f'UTC: {formatted_time}        Epoch: {epoch_time}', True, (255, 255, 255))
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                exit()
            elif event.type == pygame.KEYDOWN:  # Appui long sur touche
                if event.key == pygame.K_RIGHT:
                    ...
                    has_moved = True
            elif event.type == pygame.KEYUP:  # Relacher touche : "n" pour next, "p" pour previous
        ...
        # Handle fast forward and rewind with "space bar"

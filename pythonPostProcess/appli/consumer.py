...
def consume_data(queue: multiprocessing.Queue, do_log: bool = True, log_filename: str = None):
    ...
    if log_filename is None:
        now = datetime.now().strftime("%d-%m-%Y_%Hh%M_%Ss")
        log_filename = f"D:/Documents/Mémoire/pythonPostProcess/Sessions Logs/log_{now}.csv"
...
# Initialize Pygame : crée le GUI et la grille
    ...
    if do_log:
        output = open(log_filename, 'wb+', buffering=4096)
    try:
        while True:
            ...
            if do_log:
                output.write(
                    (str(round(time.time() * 1000)) + ";" + ';'.join(map(str, current_values)) + '\r\n').encode('utf-8'))
